...
// 03/17/07
// Changed namespace
namespace OpenMVVM.Samples.Basic.ViewModel.Helpers
{
    using System;
    using System.IO;
    using System.Text;
    ...
    public class MD5 : IDisposable
    {
        ...
        /// </summary>
        private readonly uint[] count = new uint[2];
        ...
        public virtual byte[] Hash
        {
            get
            {
                if (this.State != 0) throw new InvalidOperationException();
        ...
        {
            if (hashName == "MD5") return new MD5();
        ...
        protected virtual void HashCore(byte[] input, int offset, int count)
        {
            ...
            if ((this.count[0] += (uint)count << 3) < (uint)count << 3) this.count[1]++;
            ...
            if (count >= partLen)
            {
                ...
                for (i = partLen; i + 63 < count; i += 64) this.Transform(input, offset + i);
